...
이 저장소는 승인된 한국어 MCU 안내 62개, 필요한 라이선스 고지, 사이트 디자인 자산, 아래 승인 원문과 정적 파일을 담습니다. 개발 저장소와 Git 이력은 분리되어 있으며 사내 원문·비공개 수집 영수증·개발 문서는 포함하지 않습니다.
...
상단 누적 조회수는 기존 사이트 공통 Hits 키를 유지하며 공개 페이지 방문 때 이어서 집계합니다. 버전·경로·검색어로 키를 바꾸지 않습니다. 자료별 Hits는 텍스트 또는 데이터시트 가공 본문 표시에서만 요청합니다. 검색·목록·통계 방문은 자료별 열람을 증가시키지 않습니다. 두 지표는 합산하지 않으며 고유 방문자나 독서 완료 수가 아닙니다. 자료별 통계는 수집·배포 시점 스냅샷입니다. 미집계·일부 확인을 0이나 전체로 바꾸지 않습니다. 자세한 기준과 개인정보는 statistics/와 policy/를 확인하세요.
...
[분야별 구현 패턴](https://idtenev.github.io/mist-atelier-public/patterns/)에서 장단점·사용/비추천 상황·설계 절차·실패 조건·검증 기준을 비교합니다. 첫 선정 범위이며 전체 패턴의 전수 목록은 아닙니다.
